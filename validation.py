...
def validation_urls_page(self):
    ...
    global validation_location_text_box
    ...
    next_step_button = Button(rightframe, text=" Next Page ",width = 15,command = lambda:next_page(self))
...
def open_csv_file_selection_dialogbox(self):
        ...
        if(fullfilename != ''):
             validation_location_text_box.delete(0,END)
             validation_location_text_box.insert(0,fullfilename)
...
def next_page(self):
    if(field_validation()):
        validation_dataset_path = (validation_location_text_box.get())

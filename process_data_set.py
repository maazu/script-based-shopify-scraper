...
def read_website_df_single(dataset_path):  
    df = pd.read_csv(dataset_path, index_col=False).drop_duplicates(keep='first').reset_index()
    return df
...
async def run(host_name_list,store_list): 
    ...
    try:
         ...
         df.to_csv("test_csv"+".csv",index =False, encoding ="utf-8-sig")
         print(str("d") + " Data downloaded sucessfully into csv.............")
    except Exception as e:
          print(e)
    print(responses)
    return task
...
def start_sending_request(host_name_list,store_list):
